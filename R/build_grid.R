...
build_grid <- function(x) {
  ...
  if(any(grepl("label", names(x)))){
    ...
    vp_width <- sum(label_coord$lab_nd_width[1], vp_width)

    nodes_coord$x <- (vp_width - nodes_coord$x + label_coord$lab_nd_width[1])/vp_width
    
  }else{
    nodes_coord$x <- (vp_width - nodes_coord$x)/vp_width
  }
