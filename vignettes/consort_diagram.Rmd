...
The package has some default settings for the line width, color, arrow type and length. These can be easily changed by setting the global options. For example, you can set `options(consort_arrow_gp = gpar(col = "red", lwd = 2))` to change the arrow graphical parameters. The following options are available:
-   `consort_arrow_gp`: arrow graphical parameters, default is `gpar(col = "black", lwd = 1)`.
-   `consort_arrow_length`: arrow length, default is 0.1.
-   `consort_arrow_type`: arrow type, default is "closed". See `?grid::arrow` for more details.
-   `consort_pad_u`: padding between boxes, default is 3.
-   `consort_bullet`: bullet symbol for the side box, default is "\u2022". 
-   `consort_txt_gp`: a `gpar` object for the text, default is `gpar(cex = 0.8)`. This will be overridden by the `cex` argument in `consort_plot` if provided.
-  `consort_box_gp`: a `gpar` object for the box, default is `gpar(fill = "white", col = "black")`. 
...
options(consort_txt_gp = gpar(cex = 0.8))
