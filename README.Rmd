...
[![CRAN download](https://cranlogs.r-pkg.org/badges/grand-total/consort)](https://cran.r-project.org/package=consort)
...
As the `grid` plotting is not very ideal, calculation of the coodinates for the nodes are not easy job and tried my best. Feel free to PR if you want to improve. Or you can produce `Graphviz` plot by setting `grViz = TRUE` in `plot`. This will use `DiagrammeR` to print the plot. The plot is ideal for shinny or HTML output. 
